...
async function checkUpdates({ force = false }: { force?: boolean } = {}) {
  ...
  // Fork delta (f9f6a49a28) re-applied on upstream's API-first check (sync
  // 2026-09-11): passive checks follow the OFFICIAL repo — never a fork
  // origin — so a fork install's in-app prompt fires when upstream moves.
  // resolvePassiveUpdateSource's compareUrl is always the official HTTPS URL.
  const upstreamUrl = await getRemoteUrl(updateRoot, 'upstream')
  const slug = githubRepoSlug(resolvePassiveUpdateSource({ originUrl, upstreamUrl }).compareUrl)
