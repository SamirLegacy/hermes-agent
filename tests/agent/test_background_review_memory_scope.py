...
Fork union contract (sync 2026-09-11): the DEFAULT whitelist is EMPTY —
unattended background review is candidate-ledger only (fork hardening). The
expanded memory/skills/read-only-file whitelist below applies ONLY to attended
reviews (``/refine``, ``explicit=True``), so these scope tests pass
``explicit=True`` to exercise the expanded set.
"""
...
class TestReviewToolWhitelistScope:
    def test_skill_only_review_omits_memory_tool(self):
        whitelist, _extra = bg._review_tool_whitelist(
            _review_agent(), None, review_memory=False, explicit=True)
    ...
    def test_memory_review_keeps_memory_tool(self):
        whitelist, _extra = bg._review_tool_whitelist(
            _review_agent(), None, review_memory=True, explicit=True)
        assert "memory" in whitelist

    def test_memory_disabled_profile_stays_memory_free(self):
        whitelist, _extra = bg._review_tool_whitelist(
            _review_agent(memory_enabled=False, user_profile_enabled=False), None,
            review_memory=True, explicit=True)
        assert "memory" not in whitelist
    ...
    def test_default_scope_is_memoryless_fail_closed(self):
        ...
        # skill review that used memory nobody asked it to touch. Fork hardening:
        # unattended (non-explicit) reviews get NO tools at all — candidate-ledger only.
        whitelist, _extra = bg._review_tool_whitelist(_review_agent(), None)
        assert whitelist == set()

    def test_unattended_memory_trigger_stays_empty(self):
        # review_memory=True alone must NOT expand the whitelist: only attended
        # (/refine, explicit=True) reviews get the expanded set (fork union contract).
