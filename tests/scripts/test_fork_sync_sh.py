...
def _make_repo_fixture(tmp_path: Path, pkg_version: str = "0.17.0"):
    """A minimal npm-workspaces repo: root package-lock.json +
    apps/desktop/package.json (the layout the pack step asserts)."""
    repo = tmp_path / "repo"
    desk = repo / "apps" / "desktop"
    desk.mkdir(parents=True)
    (repo / "package-lock.json").write_text(
        '{"name": "hermes-agent", "lockfileVersion": 3, "version": "%s"}\n' % pkg_version
    )
...
def test_bundle_pack_ok_with_stubbed_npm(tmp_path):
    ...
    assert (repo / "node_modules" / ".fork-sync-lock-hash").exists()
