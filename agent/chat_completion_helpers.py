...
def _is_anthropic_wire_url(url: str) -> bool:
    ...
    return (
        url.rstrip("/").lower().endswith("/anthropic")
        or base_url_hostname(url) == "api.anthropic.com"
        or (
            base_url_hostname(url) == "api.kimi.com"
            and url.rstrip("/").lower().endswith("/coding")
        )
        # The kimi.com/coding host pair serves the Anthropic-native wire too
        # (fork carry-over; matches the primary path, #32243, #49247).
    )
