...
class TelegramAdapter(BasePlatformAdapter):
        ...
        for _attempt in range(_max_connect):
            ...
            try:
                ...
                if rebuild_app and _attempt < _max_connect - 1:
                    ...
                    # Re-register ingest-accounting + PTB error handlers FIRST on the rebuilt app.
    ...
    async def _cancel_pending_delivery_tasks(self) -> None:
        ...
        # (#83878). Discard pending sources explicitly in that case, counted
        # against the fork's drop accounting so the loss stays quantified.
        if self._is_permanent_fatal():
            n_pending = len(self._pending_text_batches) + len(self._pending_photo_batches) + len(self._media_group_events)
            if n_pending:
                self._updates_dropped_total = getattr(self, "_updates_dropped_total", 0) + n_pending
                logger.warning(
                    "[Telegram] Non-retryable fatal teardown; discarding %d pending inbound batch(es) (dropped_total=%d)",
                    n_pending,
