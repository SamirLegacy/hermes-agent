...
cmd_pack() {
  ...
  # npm-workspaces layout (upstream 2026-09): the lockfile lives at the repo
  # ROOT and desktop is one workspace; apps/desktop's own prebuild asserts a
  # root install (scripts/assert-root-install.mjs), so ci must run at root.
  [ -f "$REPO_ROOT/package-lock.json" ] || { SUMMARY="root package-lock.json missing (npm workspaces layout)"; return 2; }

  # Conditional npm ci: only when the root package-lock.json changed since the
  # last successful ci (state hash kept inside root node_modules) or
  # node_modules is absent — a merge that did not touch the lock must not pay
  # a full reinstall.
  local lock_hash state="$REPO_ROOT/node_modules/.fork-sync-lock-hash" ci_needed=0
  lock_hash=$(hash_file "$REPO_ROOT/package-lock.json")
  if [ ! -d "$REPO_ROOT/node_modules" ] || [ ! -f "$state" ] \
     || [ "$(cat "$state" 2>/dev/null)" != "$lock_hash" ]; then
    ci_needed=1
  fi
  if [ "$ci_needed" -eq 1 ]; then
    printf '%s\n' 'npm ci at repo root (root package-lock changed since last pack or no prior state)'
    ( cd "$REPO_ROOT" && nice -n 15 npm ci ) || { SUMMARY="root npm ci failed"; return 24; }
