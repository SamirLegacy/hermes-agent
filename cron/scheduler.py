...
class _CronAgentSetup:
    ...
    # Fork: cron.service_tier / agent.service_tier fast-mode resolution
    # (computed in _resolve_cron_agent_setup, consumed by _construct_cron_agent).
...
def _resolve_cron_agent_setup(job: dict, job_id: str, job_name: str, jc) -> _CronAgentSetup:
    ...
    # cron.reasoning_effort / cron.service_tier win over the agent.* keys so
    # scheduled work can run on a deeper reasoning tier than interactive
    # chat. Reasoning resolution itself happens below (per-job pin >
    # cron.reasoning_effort > _resolve_job_reasoning_config).
    from hermes_constants import parse_reasoning_effort

    raw_cron_cfg = _cfg.get("cron", {})
    cron_cfg: dict = raw_cron_cfg if isinstance(raw_cron_cfg, dict) else {}
    ...
    setup.service_tier = service_tier
    setup.request_overrides = request_overrides

    # Fork setting: a global cron.reasoning_effort overrides config-level
    # resolution so scheduled work can run on a deeper tier than chat —
    # but a per-job pin (validated, most specific) wins outright, exactly
    # like upstream's _resolve_job_reasoning_config doctrine.
    cron_effort = str(cron_cfg.get("reasoning_effort") or "").strip()
    if cron_effort and job.get("reasoning_effort") is None:
        setup.reasoning_config = parse_reasoning_effort(cron_effort)
    else:
