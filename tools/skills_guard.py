...
def scan_file(file_path: Path, rel_path: str = "") -> List[Finding]:
    ...
    # Markdown downgrade (fork carry-over): documented command idioms in .md
    # docs are scored one notch lower — see _DOC_CONTEXT_DOWNGRADE.
    ...
    for pattern, pid, severity, category, description in _COMPILED_THREAT_PATTERNS:
        for i, line in enumerate(lines, start=1):
            if i not in docstring_lines and pattern.search(line):
                text = line.strip()
                findings.append(Finding(pid, _DOC_CONTEXT_DOWNGRADE.get(pid, severity) if is_markdown else severity,
                                        category, rel_path, i,
