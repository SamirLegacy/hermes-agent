...
from hermes_cli.update_cmd_git import (  # noqa: F401
    ...
    _discard_lockfile_churn, _ensure_non_trampoline_git, _get_origin_url, _get_remote_url,
    _git_is_trampoline,
    _has_upstream_remote, _is_fork, _is_official_repo_url, _locate_real_git,
    _mark_skip_upstream_prompt, _normalize_remote_url,
...
def _pull_updates(
    ...
    keep_stash, upstream_sync_pre_sha=None):
    ...
    ``upstream_sync_pre_sha``: when the fork upstream-sync already advanced HEAD this phase,
    the rollback anchor must be the pre-SYNC sha so a syntax-error rollback also undoes the
    sync merge, not just this ff-merge."""
    ...
    pre_pull_sha = upstream_sync_pre_sha or _capture_head_sha(git_cmd, _m().PROJECT_ROOT)
    try:
...
class _CheckoutPlan:
    ...
    # Pre-sync HEAD when the fork upstream-sync advanced the checkout: the rollback
    # anchor for the pull phase must predate the sync merge, not just the ff-merge.
    upstream_sync_pre_sha: "str | None" = None
...
def _prepare_checkout_for_update(
    ...
    return _CheckoutPlan(
        ...
        upstream_sync_pre_sha=upstream_sync_pre_sha)
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        pre_pull_sha = _pull_updates(
            ...
            keep_stash=opts.keep_stash, upstream_sync_pre_sha=_plan.upstream_sync_pre_sha)
