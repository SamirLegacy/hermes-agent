...
def _run_agent(
    ...
    # be reopened before the agent can stamp a new lifecycle boundary. session_db is opened
    # here (upstream resume-first ordering) and the try below spans from agent construction
    # through chat so it is always closed (fork close guarantee).
    ...
    # Fork preflight fallback (fork delta re-applied on upstream's resume-first ordering,
    # sync 2026-09-11): if the configured default cannot authenticate (including a
    # valid-but-rate-limited Codex subscription), advance through the configured provider
    # chain here. Explicit --model/--provider requests remain provider-isolated so operator
    # diagnostics do not silently exercise a different backend.
    ...
    # The try spans agent construction (not just ``chat``) so the SQLite store opened above
    # (early, for resume resolution) is always closed — including when ``AIAgent(...)`` itself
    # raises on a provider/config error. The one-shot exit path hard-exits via os._exit and
    # skips finalizers, so an un-closed connection here would leak.
    #
    # ``_fb`` is intentionally NOT re-read here: the preflight block above already loaded the
    # chain and sliced off any entries it consumed, so the agent must inherit exactly that
    # remainder.
    agent = None
    try:
        agent = AIAgent(
