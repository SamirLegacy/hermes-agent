...
def _run_agent(
    ...
    #
    # ``_fb`` is intentionally NOT re-read here: the preflight block above
    # already loaded the chain and sliced off any entries it consumed, so the
    # agent must inherit exactly that remainder.
    agent = None
    try:
        agent = AIAgent(
