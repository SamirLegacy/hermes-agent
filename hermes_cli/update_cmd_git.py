...
def _get_remote_url(git_cmd: list[str], cwd: Path, remote: str) -> Optional[str]:
    ...
    return _git_stdout(git_cmd, ["remote", "get-url", remote], cwd)
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, *, assume_yes: bool = False, input_fn=None) -> dict:
    ...
    Fetches only upstream/main: a bare fetch drags in thousands of
    auto-generated branches.
    """
    from hermes_cli.update_cmd import (
        _add_upstream_remote, _capture_head_sha, _count_commits_between, _get_remote_url,
        _has_upstream_remote, _is_official_repo_url, _mark_skip_upstream_prompt,
        _no_prompt_git_kwargs, _should_skip_upstream_prompt, _sync_fork_with_upstream)
    ...
    # Fetch upstream main only: this sync compares upstream/main with HEAD, so
    # there's no reason to pull every upstream ref — a bare fetch drags in
    # thousands of auto-generated branches.
    print()
