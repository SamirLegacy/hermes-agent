...
def _collect_gateway_skill_entries(
    ...
    if priority_names:
        ...
        def _entry_prio(entry: tuple[str, str, str, str]) -> int | None:
            final_name, _desc, _cmd_key, raw_name = entry
            index = _prio_index.get(raw_name)
            if index is None:
                index = _prio_index.get(final_name)
            return index

        skill_entries = [
            entry
            for _i, entry in sorted(
                enumerate(skill_entries),
                key=lambda item: (0, _entry_prio(item[1]))
                if _entry_prio(item[1]) is not None
                else (1, item[0]),
            )
        ]
