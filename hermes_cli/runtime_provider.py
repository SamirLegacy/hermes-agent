...
def _pool_entry_mode_and_url(provider, entry, model_cfg, effective_model, base_url) -> tuple:
    ...
    if provider == "kimi-coding":
        # Fork intent: resolve the Kimi-for-Coding endpoint through the
        # auth-aware helper (regional routing + KIMI_BASE_URL override)
        # instead of trusting a stale pool/registry URL.
        pconfig = PROVIDER_REGISTRY.get(provider)
        default_url = base_url or (pconfig.inference_base_url if pconfig else "")
        base_url = auth_mod._resolve_kimi_base_url(
            _pool_entry_api_key(entry),
            default_url,
